...
> Note: `meeting-summarize` lives on the `meeting-summarization` branch and will be added to this list once that branch merges to `main`.
...
When `meeting-summarization` branch merges to `main`, resolve conflicts as follows:

| File | Resolution |
|------|-----------|
| `CLAUDE.md` | Use `plugin-versioning` version (this file — it is a superset) |
| `.claude-plugin/marketplace.json` | Add `meeting-summarize` plugin entry from that branch |
| `release-please-config.json` | Add `meeting-summarize` package entry |
| `.release-please-manifest.json` | Add `"plugins/meeting-summarize": "1.0.0"` |
